...
.factory('TrackerRulesFactory', function($q,MetaDataFactory,$filter,MetadataService){
    return{                
        getRules : function(programUid){            
            var def = $q.defer();
            MetadataService.getAllConstants().then(function(constants) {
                MetadataService.getProgramIndicatorsByProgram(programUid).then(function(pis){
                    var variables = [];
                    var programRules = [];
